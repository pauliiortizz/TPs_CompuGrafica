...
#ifdef MOUSE_control
    mouseY = (1.0 - 1.15 * iMouse.y / iResolution.y) * 0.5 * PI * sensitivity;
    if(iMouse.y < 1.)
#endif
#ifdef CAMERA_POS
...
#endif
#ifdef MOUSE_control
    mouseX += -(iMouse.x / iResolution.x) * 2. * PI * sensitivity;
#endif
    
#ifdef CAMERA_FAR
    vec3 eye = (2. + CAMERA_FAR) * vec3(cos(mouseX) * cos(mouseY),
                                        sin(mouseX) * cos(mouseY),
                                        sin(mouseY));
#else
    vec3 eye = (4.0 * iZoom) * vec3(cos(mouseX) * cos(mouseY),
                                    sin(mouseX) * cos(mouseY),
                                    sin(mouseY));
#endif
...
void main() {
    // Convertir coordenadas de [-1,1] a píxeles
